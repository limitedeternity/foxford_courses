...
2. Запусти "python install.py". Если ты на Windows, то импортируй *longpath_fixer.reg*, на всякий случай.
...
Добавлено сохранение теории. Вырезан модуль ДЗ.
...
Добавлено возобновление загрузки на случай, если что-то пойдет не так, чтобы не ждать по-новой. Необходимо просто выбрать после перезапуска пункт меню, в зависимости от того, где оборвалось: если на теории, то выбирать первый пункт (снова). Если на видео, то, чтобы пропустить теорию, которая сохранена, выбирай второй пункт.
